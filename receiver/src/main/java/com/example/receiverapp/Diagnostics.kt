...
object Diagnostics {
    ...
    fun log(message: String) {
        ...
        executor.execute {
            try {
                ...
                if (file.length() > MAX_LOG_BYTES) file.writeText(file.readText().takeLast(MAX_LOG_BYTES / 2))
    ...
    fun export(context: Context) {
        ...
        val report = buildString {
            appendLine("AndroidUdpNotifier Receiver diagnostics")
            appendLine("Version: ${BuildConfig.VERSION_NAME} (${BuildConfig.VERSION_CODE})")
            appendLine("Device: ${Build.MANUFACTURER} ${Build.MODEL}; Android ${Build.VERSION.RELEASE} (SDK ${Build.VERSION.SDK_INT})")
            appendLine("Sender last seen: $age")
            appendLine("Sender address: ${maskIp(prefs.getString("LAST_SENDER_IP", null))}")
            appendLine("\nLast crash:\n${crashFile(context).takeIf { it.isFile }?.readText() ?: "none"}")
            appendLine("\nRecent events:\n${logFile(context).takeIf { it.isFile }?.readText() ?: "none"}")
        }
        val file = File(context.filesDir, "diagnostics/receiver-diagnostics.txt")
        file.parentFile?.mkdirs()
        file.writeText(sanitize(report))
